...
dset = datasets.dataset_digits(n_class=5)
...
nn_model = MLP.MLP(layer_sizes=np.array([64, 10, 10, 5]),f_activation='relu',f_output='softmax', sigma=1.5)

# Train the network on the sample dataset for 500 epochs using learning rate 
# of 0.1
nn_model.train(dset.x,dset.target_y,num_epochs=1000,alpha=0.1)
